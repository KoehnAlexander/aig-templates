...
%TODO Add general introduction to the topic. Motivate possibilistic logic and only then give overview on the structure of this paper.
%TODO add citation

Reasoning under uncertainty is a fundamental problem in artificial intelligence and knowledge representation. Classical logic, while expressive and well-structured, lacks the ability to handle incomplete or uncertain information effectively. Several approaches have been proposed to address this issue, including possibility theory \cite{halpern1990}, fuzzy logic \cite{zadeh1965}, and possibilistic logic \cite{dubois1994}. The foundation of possibility theory was laid by Zadeh in 1978 \cite{zadeh1978} as an extension of fuzzy set theory \cite{zadeh1965}, aiming to provide a qualitative framework for handling uncertain information. Building upon Zadeh’s work, Dubois and Prade formulated possibilistic logic in the late 1980s and early 1990s \cite{dubois1991, dubois1994} as a means to integrate possibility theory into formal logic systems. \\
...
In the following basic concepts of possibility theory are introduced. Possibility theory is explained in detail in \cite{dubois2023possibility}, \cite{zadeh1978}. First the possibility distribution is introduced and second the basic set functions of possibility theory which model the possibility and necessity of events. \\
\begin{definition}
...
If we want to know if the event $ A $ occurs we can respond in possibility theory with two measures calculated from the possibility distribution to $ A $.  $ A $ is a subset of states. The two measures are the possibility and the necessity of an event, see \cite{dubois2014possibilistic}. \\ %TODO Clarify the sentence
\begin{definition}
...
\end{equation}
If we want to infer from the knowledge base, we can only look on the formulas with a certainty of at least $ \alpha $. This means that a inference is only valid if it is valid with the $ \alpha $-cut of a knowledge base. \\
...
%The semantics of possibilistic logic, as introduced in \cite{dubois2014possibilistic}, is based on the concepts of possibility distributions, weak possibility measures, and strong necessity measures. \\
